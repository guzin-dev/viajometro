...
function Countrys() {
  ...
  useEffect(() => {
    ...
    filteredList = filteredList.filter((item) => item.nome.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <main className="font-montserrat bg-[#111827] text-white flex flex-col items-center min-h-screen overflow-x-hidden">
      <section className="p-4 px-6 flex max-w-[90rem] items-center w-full sticky top-0 bg-[#111827]">
        <div className="w-full border rounded-md p-2 flex items-center gap-2">
          <span className="font-materialIcons">search</span>
          <input
            onChange={(e) => {
      ...
      <section className="flex w-full px-4 max-w-[90rem]">
        <div className="max-w-[90rem]">
          <div className="flex flex-wrap">
            {countriesList.map((country) => {
              return <CountryCard data={country}></CountryCard>;
            })}
          </div>
        </div>
      </section>
